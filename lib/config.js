...
// Returns true when sold_out is set to true in Edge Config (or SOLD_OUT env for local dev).
export async function isSoldOut() {
    if (process.env.SOLD_OUT === 'true') return true;
    try {
        const value = await get('sold_out');
        if (typeof value === 'boolean') return value;
    } catch {
        // Edge Config not connected — default to not sold out
    }
    return false;
}
