...
//   price_max                    60       sliding scale up to here;
//                                         set equal to the min for one fixed price
//   price_step                   5        € between the sliding-scale steps
//   max_quantity                 10       tickets per order
...
const ENV_OVERRIDES = {
    ...
    price_step: process.env.EDGE_PRICE_STEP,
    max_quantity: process.env.EDGE_MAX_QUANTITY,
    group_deal: process.env.EDGE_GROUP_DEAL,
    sold_out: process.env.EDGE_SOLD_OUT,
    sounds_enabled: process.env.EDGE_SOUNDS_ENABLED,
...
/**
 * The checkout's numbers, in €. `max === min` means a single fixed price;
 * `max > min` means a sliding scale the buyer picks from in `step` increments.
 * Both the form and the API validate against this, so they cannot drift.
 */
export async function getPricing() {
    ...
    return {
        ...
        step: asNumber(await read('price_step')) ?? EVENT.priceStep,
        maxQuantity: asNumber(await read('max_quantity')) ?? EVENT.maxQuantity,
    };
}
...
/** Button click sounds. Read once in the root layout, see lib/sounds.js. */
export async function areSoundsEnabled() {
    const value = await read('sounds_enabled');
    return typeof value === 'boolean' ? value : EVENT.sounds;
}
